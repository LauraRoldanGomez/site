...
class(ourdata_as_list[1])
typeof(ourdata_as_list[1])
# 2. What is the class of ourdata_as_list[[1]]?
class(ourdata_as_list[[2]])
typeof(ourdata_as_list[[1]])
...
colnames(mtcars)[1:3]<-c("miles per gallon","cylinders","displacement")
colnames(worms)[1]<-c("A") # Careful, you can overwrite your data... Good practice moment: when playin with your data, save it under a different name. I use a name with a number attached so that I know the sequence of actions
dt1 <- colnames(worms)[1]<-c("A")

# Try out what happens if you try to add a new column of a length that is less than 32.
mtcars$new_column<-1:25
...
new_sepal <- data.frame(iris$Sepal.Length)
sepal_big<- subset(iris, Sepal.Length>4.6)
#-----------------------------------------#
#-----------------------------------------#
...
class(iris_matrix)
iris_heatmap <- iris[order(iris$Sepal.Length),]
iris_heatmap <- data.matrix(iris_heatmap)
heatmap(iris_heatmap)
